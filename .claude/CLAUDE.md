...
**2.12's mechanism is BUILT and runs in `report` mode; no number has changed yet.** `src/backfill.py` + `cross_window_order_backfill: off|report|apply` ([D59](../docs/06-DECISIONS.md#d59)). `report` is the default and is **manifest-neutral by construction** — its outputs are a tie-out INFO row, a `Cross-window Orders` exceptions sheet and log lines, and INFO rows never become variances (`consume_tieout` promotes only a `BREACH`) while `stage_row_counts` comes from `make_golden.STAGE_TARGETS`, an explicit function list this does not touch. **The money gate re-ran all eight windows under `report` at zero tolerance: no cell moved.** What remains is the flip to `apply`, which moves cells and needs the per-window delta stated in advance.
...
- `ingest.read_files` / `export_files` were **extracted** from `read_parts` (name, signature and behaviour unchanged) so borrowing reads through the same rules every verified number came from. `backfill.predecessor_labels` is one rule over two candidate sources — directories on the CLI, window labels from Postgres in `materialize_predecessor_orders`. Do not add a second spelling of either.
