...
# Do NOT add `-w` from a tool call: it holds the pipe after the server is ready, the
# call times out, and the kill takes the server with it (measured 2026-08-21 —
# `0xC0000142` in server.log). Start it via PowerShell `Start-Process … -WindowStyle
# Hidden` and poll pg_isready instead (docs/14, "Before you start").
...
# `enqueue-master` (A4) queues a month-end master WITHOUT re-running any window —
# the break-glass twin of POST /months/{month}/master, for when the api is down.
"$PY" -m service.admin job list [--state leased]
"$PY" -m service.admin job reclaim
"$PY" -m service.admin job enqueue-master --month 2026-07

# Phase 6 (2026-08-20): who downloaded which workbook (C12), and retention by
# hand (C10 — the worker sweeps hourly on its own; scratch 14d, run-log DB
# mirror 90d, dead sessions 30d, disk-free warning).
"$PY" -m service.admin audit downloads [--run N]
...
# thirteen tables. `service/api.build_app` seeds them on first boot; this is the same
...
# The real .env lives OUTSIDE the tree since Phase 6.9 (C13): this folder is
# OneDrive-synced and gitignore does not stop a sync client. Two optional
# profiles: `--profile ingress` (nginx: TLS, pinned X-Forwarded-For, per-IP rate
# limits; needs RECON_INGRESS_CERTS — deploy/ingress/make-self-signed.sh) and
# `--profile backup` (nightly pg_dump; the restore drill is
# tools/db_restore_drill.sh and docs/09 keeps its log).
...
Expected today: **`1065 passed, 3 skipped, 4 deselected`** in ~6:39 with `RECON_TEST_DATABASE_URL` set and `RECON_REQUIRE_CLIENT_DATA=1` (measured 2026-08-21 after D5, `-m "not slow"`, IO cache OFF); the ~600 service tests skip loudly with no database. **The ~14:35 this line carried for the 1018 figure was a contended-machine measurement** — taken right after an 8-window money-gate run — and this tree now runs the same suite in half that, with the money gate itself at 3:37 against a documented ~11 min. Treat the wall clocks here as machine-state, not as a property of the suite; the two `test_a_sanitized_renamed_window_produces_the_committed_golden` cases are still ~67% of it whatever the absolute figure. This line said `630` from 2026-08-17, `731` until 1.6 replaced the config-editor tests, `741` until Phases 2/4/5, `816` until Phase 3 added `date_formats` (which fans out the config-parametrized tests), `tests/test_master_summary.py` (8) and `tests/service/test_month_master.py` (8), `840` until the register fix pass added 27 tests (`tests/test_atomic_writes.py` 6, `tests/test_lazada_promo_pairing.py` 5, `tests/test_order_coverage.py` 7, three 2.9 regressions, three pin-event and three artifact-digest service tests) plus 7 file-parametrized lint cases over 5 new files, `874` until C11/C12 added 22 (`tests/service/test_uploads.py` +10 for the date door, `tests/service/test_order_index.py` 12) plus 3 lint cases over 2 new files, and **`930` until C13's backfill landed** (25 in `tests/test_backfill.py`, 3 in `tests/service/test_materialize.py`, 3 lint cases) — the `899` written here on 2026-08-19 was measured before C13 and was stale, not wrong. `930` until the borrow-path fix added 7 (4 backfill — alias, coercion, and the report/apply failure-policy pair; 1 from splitting the predecessor-digest test into its two modes; 2 for the artifact download's streamed branch), and `937` until B1 added the 3 falsification tests that drive D59's safety property through the real explode and tie-out. `941` until A14 added 16 (`tests/test_contract_buckets.py` 11 — hard-stop accessors, layout and rate-trio guards; `tests/service/test_config_rows.py` +5 — the two bucket tables and the numeric-list scalar) — the `940` this line carried for one day was measured stale by one against the same tree, not wrong. **`957` until M8 Phase 6 added 25**: 18 tests (`tests/service/test_migrate_lock.py` 2, `test_retention.py` 6, `test_observability.py` 6, download-audit +2 in `test_api.py`, size-cap +2 in `test_uploads.py`) plus 7 file-parametrized cases over the two new service modules (2 deletability, 5 io-boundary). **`982` until the workflow-holes pass (A4/D1/D2/D3) added 36**: `tests/test_partial_roster.py` 9 (the legacy-blanket contract, per-store relaxation, the workbook stamp), `test_exceptions_and_board.py` +12 (7 dispositions, 5 board-union/months), `test_month_master.py` +6 (chained persisted ×2, the manual route ×4), `test_worker.py` +3, `test_api.py` +2, `test_uploads.py` +2, `test_admin_cli.py` +2. **`1018` until A9 (2026-08-21) added 13**: `tests/test_config_defaults.py` 12 (the AST walk over `src/`'s remaining `settings.get(key, literal)` defaults, its negative control, and the `require`/`vat_default` refusals) plus 1 from `test_the_keys_whose_absence_changes_behaviour_are_caught` parametrizing over three required keys instead of two. That accounts for 13 of the 14; the measured total was `1032`, so the `1018` was one low against this tree — the third time this line has been stale-by-one, and the reason it now says: treat a mismatch as a stale doc first and a regression second, but check. **`1032` until D12 (2026-08-21) added 15**: `tests/test_brand_mapping.py` 11 (the norm_store matching rule, the bare-string refusal, and one case that runs against `config/settings.yaml` itself because every synthetic fixture agrees with itself), `tests/service/test_config_rows.py` +3 (the roster rule for brand rows, Lazada exempt), and **one io-boundary case**, because that lint is parametrized per FUNCTION in `src/`: `ingest` gained `norm_store`/`store_brands`/`brand_map` while `pipeline` lost `norm_store` and `master_summary` lost `parse_brand_map`, net +1. Worth knowing — moving a function between `src/` modules moves this number. **`1047` until D7 (2026-08-21) added 10**: `tests/service/test_uploads.py` +5 (the store-preview route), `tests/test_upload_store_control.py` 4 (the three sides of the `store:<filename>` contract, joined at text level because there is no browser), and **one io-boundary case for the new test FILE** — that lint's second parametrization covers every file under `tools/`, `tests/` and `service/`, so adding a test file moves this number too. Between them the two parametrizations mean the count tracks the shape of the tree, not just the number of assertions.
...
**Three platform paths, two shapes.** TikTok and Shopee share `ingest → stitch → classify → calculate` (orders + income files). Lazada is a self-contained vertical in `src/lazada.py` (a fee-event ledger, no order files). Its column maps, sheet names and filename regex were hardcoded there until **M8/1.7**, and are now `column_maps.lazada` / `sheet_names.lazada` / `store_from_filename.lazada` like every other platform's; `lazada.column_map()`, `sheet_name()` and `store_pattern()` read them and hard-stop rather than falling back to a copy. Its bucket names followed on **2026-08-20** (A14, the last of the move): `fee_buckets.lazada` names the revenue bucket and the promo buckets, read by `lazada.revenue_bucket()` / `promo_buckets()` with the same hard-stop posture, and `revenue_lines` takes `settings`. The invoice-bucket lists and `VAT_RATES` left `finance_template.py` the same day (`invoice_buckets.<platform>`, `vat_factors.rates`); the workbook TAB layout stays code deliberately — a configured bucket the template has no tab for hard-stops, and Shopee/Lazada hard-stop on any rate list other than the trio their control-row geometry is wired for. **`store_to_brand` was the last of it (2026-08-21, D12/D65)**: it is per platform now, carries `brand`/`confidence`/`note`, and `config/brand_map.csv` — the second mapping, which only the month-end master read — is deleted. Both readers match through `ingest.norm_store`, which is the whole fix: the CSV's keys were normalised spellings against roster spellings, so the exact `.map()` it replaced resolved 2 of 42 rows. `norm_store` itself moved `pipeline` -> `ingest` (re-exported) so `derive_brand` could reach it. Brand is NOT the invoice bucket: `store_to_brand` says whose revenue it is, `invoice_buckets` says which tab it lands on.
...
## Current work: M8 COMPLETE (Phases 1–6, 2026-08-20) — what to do next is in `docs/10-ROADMAP.md`

**Last completed (2026-08-21): register D5 — format drift is visible where it happens.** Details in `docs/12-CHANGE-HISTORY.md` and decision **D67**; this closes the A9/D12/D7/D5 plan. Two deterministic halves: the door records the headers a file arrived with (migration `023` — `pii_columns_dropped` already held EVERY dropped header despite its name, so `kept_columns` completes the original row) and reports the ones the contract does not name **with `KNOWN_PII` subtracted**; the window screen maps one to a canonical field as an ordinary config proposal. **The required-field check is per window-and-kind, never per file** — `read_parts` checks the concatenation, and a part file with fewer columns is legitimate (July produced nine), so a per-file refusal would break healthy windows; it appends to `problems`, which makes `ready` false. `checked: false` means UNMEASURED, not clean (Lazada has no required set; pre-`023` uploads recorded no headers). `store` is excluded from the required set because it comes from the filename (D6). **No suggested mapping** — that would be the first model call in this system and it needs PII-bearing cell values; deferred with its constraints in `docs/10-ROADMAP.md`. No `src/` change, so no golden could move and the money gate was deliberately NOT run.

**Previously (2026-08-21): register D7 — the per-file store correction is reachable.** Details in `docs/12-CHANGE-HISTORY.md` and decision **D66**. `POST /uploads/store-preview` takes **filenames only** (`user`, changes nothing) and answers per file with the derived store, its canonical form, the uniform name it will carry and whether that storefront is on THIS window's roster; the upload form renders a row per chosen file with a correction picklist. The defect was a three-sided contract missing one side — `POST /uploads` took `store` and `actions.ts` posted `store:<filename>` since M6, and nothing rendered an input — so `tests/test_upload_store_control.py` now joins all three, text-level, because **there is still no browser automation** (E2). Do not add a filename regex to `web/`: the derivation is `naming.store_of`, and a second definition of what decides whose revenue a file becomes is the worst drift available here (D6). No `src/` change, so no golden could move and the money gate was deliberately NOT run.

**Previously (2026-08-21): register D12 — one storefront->brand mapping, in the database, retrieved like every other config.** Details in `docs/12-CHANGE-HISTORY.md` and decision **D65**; the short version is the paragraph in *Three platform paths* above. Three things worth not re-deriving. **(1) The register's "rendering brand_map.csv would change the brand of 28 stores" was wrong at the pipeline** — `derive_brand` matched exactly and the two spellings never met, so it was 2, with 40 silently unbranded; the fix is the matching rule (`norm_store` both sides), not `in_pipeline_contract`, which is dropped in migration `022`. **(2) The month-master worker was reading `load_settings(config_dir)`** — the container's baked copy — which was harmless until the brand mapping became contract and is A1 one layer over; it resolves unpinned through `_resolve_config` now. **(3) `confidence` had two vocabularies** (the CSV's `high` vs the table's `confirmed`), and left alone every row on the master's review tab would have printed red. Deliberately NOT filled in: three July Shopee storefronts (`Tolpa`, `pepsicofoods`, `xa_kho_gia_tot`) have no brand row because nobody has told us their client brand — they fall back to their own name and the master flags them UNMAPPED, and inventing one in a mapping the master invoices from is not engineering's call. Money gate: all 8 windows at zero tolerance, no cell moved (pre-stated — `brand` is functionally determined by `store`, so `classify.py`'s groupby is unchanged, and no template cell reads it).

**Previously (2026-08-21): register A9 — a code default may not disagree with the contract.** Details in `docs/12-CHANGE-HISTORY.md` (*2026-08-21 — A9*). The row named one instance (`dedupe_rows` defaulting to `True` against a contract that says `false`); planning the fix found a **second live one nobody had named** — `cross_window_order_backfill` defaulting to `"off"` against `apply`, silently reverting defect 2.12 and 2.33B VND of July recovery. **That default was safe the morning it was written and wrong by that evening**, when the mode was flipped; a default is a claim about the configured value and goes stale on its own. So: `src/config.py` `REQUIRED_SETTINGS` + `require` — no default at all for `dedupe_rows`, `drop_unmapped_columns`, `cross_window_order_backfill`, plus a refusal of the quoted `"false"` that is truthy in Python; `vat_factors.default`'s three bare `1.08` fallbacks became `config.vat_default` (A14's last residual); `service/config_render.DANGEROUS_DEFAULTS` **imports** that set rather than restating it, its own copy having called the backfill default "the SAFE direction" inside the control meant to catch it. The lasting piece is `tests/test_config_defaults.py` — an AST walk pairing every surviving `settings.get(key, literal)` in `src/` with the configured value, with a negative control so it cannot pass by matching nothing; `date_coercion` is the one allow-listed key (nothing configures it, D53). `tests/test_backfill.py`'s "unset means off" test was **asserting the bug** and is replaced by one pinning the hard stop. **Money gate: all 8 windows re-run at zero tolerance, no cell moved** (the delta stated in advance). Not touched, stated so it is not re-derived: `dayfirst.tiktok`'s `False` default is a real inversion and a dead one — `date_formats` decides for both platforms that read it (D54).

**Previously (2026-08-21): the workflow-holes pass — register A4 (remainder), D1, D2, D3, all FIXED.** Details in `docs/12-CHANGE-HISTORY.md` (*2026-08-21 — the workflow holes*) and decisions **D61/D62/D63**. The short version: `POST /months/{month}/master` + `service.admin job enqueue-master` queue a month master without re-running a window, and the chain outcome persists on `runs.chained` (migration `019`); exception dispositions exist (migration `020`) — **annotate, never hide**, `recon.user`, mandatory reason, append-only events; the board unions jobs ∪ roster declarations ∪ live uploads so uploaded-but-never-run windows appear, with `GET /months` behind a real month picker; the roster declaration names WHICH stores are absent (migration `021`, `apply_partial_roster(optional_stores)` — `None` = the legacy blanket the CLI and goldens use, pinned in `tests/test_partial_roster.py`), stale declarations are warned and amber-flagged, and a partial run **stamps the workbook** (`PV sum`/`Summary` A1/B1, Vietnamese, names the absent stores). **The stamp moved exactly the four `partial_roster: true` goldens by exactly two cells each — the one deliberate rebaseline; the four lazada goldens regenerated unmoved first.** Found on the way and recorded as **defect 2.13**, deliberately NOT fixed: `_Tab.emit` drops control cells at rows ≥ `data_start_row`, so the TikTok/Shopee `PV sum` side-block totals/verdicts have never reached the file (the verdicts survive in `checks`/run log); fixing it moves all eight goldens and needs its own pass. New-table gotcha: `exception_dispositions`/`_events` key on the fingerprint with no FK, so they are truncated EXPLICITLY in `tests/service/conftest.py` — a new keyless table must join that list or state leaks between tests.

**Previously (2026-08-20): M8 Phase 6 — deployment hardening, the final M8 phase.** Register items C3–C13, each to the boundary of what the repo can do with the hosting-side halves *named* (docs/14 Phase 6 table): advisory-locked migrations, a 512 MB upload cap (largest real export: 184 MB), worker liveness on `/healthz` (`workers_alive` — migration `017`), a download read-audit (migration `018`, `service.admin audit downloads`), retention (`service/retention.py`, hourly + CLI), JSON-line service logs + `GET /metrics` (viewer role, counts only — never a store name), D60 (the database is the config audit record; git is a checkout convenience), nightly `pg_dump` under `--profile backup` with a **performed** restore drill (recon_dev: 30 tables/1,037 rows identical — docs/09 logs it), the real `.env` moved to `%LOCALAPPDATA%\recon-deploy\`, and an nginx `--profile ingress` (TLS, pinned X-Forwarded-For, per-IP limits, `RECON_ALLOWED_ORIGINS` build arg) — `nginx -t`-verified in the real image, **not yet browsed through** (2.8's standing limit). Nothing touches `src/`. Earlier the same day: register item **A14** — the C14 step of the defect-register plan, and the last of it. `vat_factors.rates`, `invoice_buckets.<platform>` and `fee_buckets.lazada` joined the contract (migration `016`, hard-stop accessors, values verbatim); all eight goldens re-ran at zero tolerance unmoved. A window pinned to a pre-A14 config hard-stops on re-run naming the missing key — the remedy is the audited unpin/repin. Earlier the same day, defect 2.12's fix switched on (`cross_window_order_backfill: apply`) after the borrow path was found to skip `store_aliases`; July's gap against the team's master: 4,527,401,608 → 1,579,645,766 VND, no golden cell moved, and the residual needs two order exports re-pulled (`docs/16` Ask 3), not code. ~~**Next candidates:** the M6-of-the-roadmap exception-queue depth (D1) and the window index on the board (D2).~~ Both done 2026-08-21 (the paragraph above); SSO stays blocked on the tenant (C15), and defect **2.13** (the `_Tab.emit` drop) is the newest named candidate.

`~/.claude/plans/want-you-to-explore-tingly-wolf.md` is the **M1** plan, `M6-PLAN.md` is the **M6** plan, `~/.claude/plans/assess-the-open-known-luminous-tarjan.md` is the **defect-register** plan (C1–C14 + B1–B3; everything but C14 is done) and `~/.claude/plans/fix-backfill-normalization-and-review-findings.md` is the **review-findings** plan (F1–F5, all done) — all history now, and the last two record where they turned out to be wrong. The roadmap in `docs/10-ROADMAP.md` is authoritative for what comes next, and `docs/12-CHANGE-HISTORY.md` records where each plan turned out to be wrong — M6-PLAN in particular asserted a leaked password that did not exist, and claimed removing a commented config item takes its comment with it, which holds only for an EOL comment. **The polars migration was descheduled 2026-08-12** ([D25](../docs/06-DECISIONS.md#d25)) — do not propose it, do not write polars, do not reinstate a pandas "oracle" framing. pandas stays; a port is trigger-gated on instrumented thresholds in `docs/10-ROADMAP.md`.
...
**2.12 is FIXED and `cross_window_order_backfill` is `apply` since 2026-08-20** — `src/backfill.py` + `off|report|apply` ([D59](../docs/06-DECISIONS.md#d59)). It ran as `report` in production from 2026-08-19 first, deliberately, so the computation that moves money had been reporting before it was trusted with any. **The flip moved no golden cell, and that was pre-stated:** `needed` comes from `read_parts` frames on both sides, so no golden window borrows anything (`2026-05_w1`/`_s1` are first of their month, `_s2`/`_s3` have 100% own-window coverage, Lazada has no order files). Money gate re-ran all eight at zero tolerance after the flip — no cell moved, **no golden re-baselined**. On real data `2026-07_w2` applies 942,869,056 VND across 825 orders with per-order conservation **exact** (variance 0.00 on a 38.6B rebuilt total); `s2` 173,429; `s4` 1,390,095,674. A **pinned** service window keeps the mode it was pinned to, so re-running July through the worker needs a deliberate repin. `2026-07_s4` needs `--partial-roster` on the CLI for an unrelated reason: two rostered stores have order files in `s3` and none in `s4`.

- **Do not "fix" 2.12 by pooling the month's order files** — measured, that takes w2 to 183B against a reference of 40B, and the over-count is *invisible* because the explode sums quantity per `(store, order_id, sku_id, sku_name, unit_price_gross)` bucket, so a duplicate inflates an existing line rather than adding a row.
- **Borrowing takes each order from exactly ONE window, nearest predecessor first**, and never from a later window (a re-run of `w2` must not change because `w5` arrived).
- `ingest.read_files` / `export_files` **and `ingest.normalize_parts`** were extracted from `read_parts` (name, signature and behaviour unchanged) so borrowing reads through the same rules every verified number came from. Sharing only the *reading* half was a real defect: borrowed frames got no `store_aliases`, so a filename saying `Pediasure` never matched a canonical `Abbott Pediasure` and 941,081,056 VND of July recovery silently reported as zero (fixed 2026-08-20). The filename prefilter goes through `ingest.canonical_store`, which also absorbed the duplicate that lived in `service/materialize.py`. `backfill.predecessor_labels` is one rule over two candidate sources — directories on the CLI, window labels from Postgres in `materialize_predecessor_orders`. Do not add a second spelling of any of these.
- **`tools/measure_order_coverage.py` and the pipeline must agree, and their disagreement is a defect signal.** The tool reads through `read_parts`; the pipeline borrows through `read_files` + `normalize_parts`. They now state identical figures on `2026-07_w2`/`_s2`/`_s4`. That agreement is how the alias gap was found and closed — if they ever diverge again, the pipeline is wrong, not the tool.
...
- Config is edited as **rows**, not as paths into a file. `service/config_rows.py` holds the closed registry of thirteen tables and two operations (`upsert`, `delete`); `service/config_edits.py` and `service/config_schema.py` were **deleted in M8/1.6**. A table that refuses a new row says why in a sentence and the refusal quotes it. `invalidates_goldens` is a column read off the row — never inferred from a path — and `verification.verify` takes the answer rather than computing it.
