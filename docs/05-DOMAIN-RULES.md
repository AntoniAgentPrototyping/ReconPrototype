...
**The bucket lists are config, and were not always.** They are `invoice_buckets.<platform>` in the contract, read through `finance_template.invoice_buckets`, which hard-stops rather than falling back to a copy — since **A14 (2026-08-20)**. The workbook's TAB layout stays code deliberately: a configured bucket the template has no tab for is a hard stop, never a silent leak. *(This section said "there is no config file for this" until 2026-08-21, which A14 had already made false — corrected here rather than quietly, since which layer owns a rule is exactly what these docs are for.)* `config/brand_rules.yaml` carried a separate, much smaller notion — `invoice_grouping: separate | combined` per brand — and was **deleted on 2026-08-18** along with its only reader, `classify.classify()`, which had no callers of its own. The invoice bucket lists were hardcoded in `finance_template.py:90-92` at the time, so nothing about the invoice split changed then; what went was a config file that looked like it governed the split and did not. See [D11](14-PRODUCTION-READINESS.md) and [D19](06-DECISIONS.md#d19) for the deletion standard (goldens unchanged is the proof).

**Brand is a different thing from an invoice bucket, and both are now config.** `store_to_brand` is the storefront→client-brand mapping — who the revenue belongs to, read by `ingest.derive_brand` and painted by the month-end master. `invoice_buckets` is which workbook tab a store's lines land on. `store_to_brand` was empty until 2026-08-21, so every store fell back to its own name with a loud ingest warning while a *second* mapping (`config/brand_map.csv`, 60 rows) served the month-end master alone; there is one mapping now, per platform, matched through `ingest.norm_store` on both sides ([D65](06-DECISIONS.md#d65)). A storefront with no row still falls back to its own name and is still reported — three Shopee storefronts are in exactly that state on purpose, because nobody has told us their client brand.
