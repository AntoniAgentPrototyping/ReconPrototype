...
| `src/backfill.py` | An order's SKU lines, borrowed from the earlier window that exported them (defect 2.12) | Three modes on one code path — `off` / `report` / `apply` — so the mechanism that eventually moves money is the one that has been reporting in production. **One** predecessor window per order, never pooled: pooling is measured at 4.5× over-count ([D59](06-DECISIONS.md#d59)). Reads through `ingest.read_files` **and** `ingest.normalize_parts`, not a copy of the reading rules — it shared only the first of those until 2026-08-20, and the missing half silently zeroed the aliased stores' recovery. |
...
There is no shared mutable state left. Until 2026-08-19 there was: `settings["_vat_sku"]` was injected by `build_context` and read inside `calculate.py`, using the config dict as a data channel, so two concurrent runs sharing one settings dict would have cross-contaminated VAT rates. That channel and a second, undocumented one (`_masters_source` / `_masters_searched`) are gone — the values are `RunContext` fields and an explicit `vat_sku` parameter (defect 1.9). One-job-per-process still stands, on memory grounds rather than this one.

## The I/O boundary is the load-bearing structural property

File *content* I/O is confined to four modules — `ingest.py`, `lazada.py`, `config.py`, `masters.py` — plus `finance_template.write_workbook`. Everything else is frame-in/frame-out. One qualification since 2026-08-19: `backfill.py` lists directories (`is_dir`, `iterdir`) to find sibling windows, but opens nothing — every byte it reads comes through `ingest.read_files`. `IO_NAMES` in the lint does not cover the listing calls, so that boundary is held by review rather than by the test. This single property is what makes the pipeline testable without real data, embeddable in a worker, and cheap to move to another compute engine if that ever becomes necessary.
