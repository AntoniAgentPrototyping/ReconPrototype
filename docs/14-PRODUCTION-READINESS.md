...
| **D5** | Format drift has needed a developer every month tested, and nothing in the product helps a finance user absorb it | `12-CHANGE-HISTORY.md:5` | **`FIXED` (2026-08-21)** — [D67](06-DECISIONS.md#d67), in the two deterministic halves decided with the user. **The evidence:** the door records the headers a file arrived with (migration `023` — `pii_columns_dropped` already held every dropped header despite its name, so `kept_columns` completes the original row) and reports the ones the contract does not name, **PII subtracted** or the list fires on every healthy file. **The step:** the window screen maps an unknown header to a canonical field as a config proposal, reusing propose→approve→apply. The required-field arithmetic is per window-and-kind, **never per file**, because `read_parts` checks the concatenation and a part file with fewer columns is legitimate (July produced nine) — it appends to `problems`, so `ready` goes false: a window a run will refuse must not look ready. `checked: false` distinguishes *unmeasured* from *clean* (Lazada has no required set; pre-`023` uploads recorded no headers). **No suggested mapping** — that is the first thing here that would call a model, it needs to read PII-bearing cell values, and it is deferred to its own pass with the constraints recorded in `10-ROADMAP.md`. Not browser-verified (E2 stands) |
...
| **D7** | The per-file "correct the store" control is read server-side but no input with that name is ever rendered, so the documented affordance is unreachable | `web/app/actions.ts:319` vs `web/app/windows/[platform]/[period]/upload-form.tsx` | **`FIXED` (2026-08-21)** — [D66](06-DECISIONS.md#d66). `POST /uploads/store-preview` (**filenames only**, `user`, changes nothing) answers per file with the derived store, its canonical form, the uniform name it will carry, and whether that storefront is on **this window's** roster; the upload form renders a row per chosen file with a correction picklist (free text only where there is no roster — Lazada, A6). The derivation is `naming.store_of`, the pipeline's own rule — a browser-side regex would be a second definition of what decides whose revenue a file becomes (D6), and a test asserts no `.tsx` reads `store_from_filename`. Measured so the picklist cannot offer a self-refusing option: all 45 roster storefronts survive `validate_roundtrip` on every kind. A failed preview does not block the upload — the door runs the same checks. **Not browser-verified** (E2 stands): what is tested is the route (5 cases) and the field name in all three places, which is the side that went missing |
...
| **D12** | Two separate storefront→brand mappings, used by different things: `store_to_brand` in the contract is `{}` (so every store falls back to its own name with a loud warning every run), while `config/brand_map.csv` holds 60 rows and is read only by a developer script | `config/settings.yaml:463`; `src/ingest.py:589-598`; `tools/build_master_summary.py:44` (the citations in this row were stale by ~190 lines and one deleted function) | **`FIXED` (2026-08-21)** — [D65](06-DECISIONS.md#d65). One table (`config_store_brands`), one rendered key, two projections: a run reads `ingest.store_brands(settings, platform)`, the month-end master reads `ingest.brand_map(settings)`. `config/brand_map.csv`, `parse_brand_map` and `month_master.brand_map` are deleted and `in_pipeline_contract` is dropped (migration `022`). **The flag was never the fix, and measuring is what said so:** `derive_brand` matched exactly, the CSV's keys were `norm_store` spellings and the pipeline's are roster spellings, so flipping it would have rebranded **2** storefronts and silently left 40 unbranded with the contract looking full — 007's "28 stores" was measured a different way and was never what the pipeline would do. Both readers now match through `norm_store` (25/25 TikTok, 17/17 Shopee resolve), the rendered key is per platform and carries `confidence`/`note` because the master's review tab paints them, and a brand row must name a roster storefront (Lazada exempt and reported — A6). Three July Shopee onboardings are deliberately unmapped: nobody has told us their client brand. Found on the way: the month-master worker read `load_settings(config_dir)`, so an edited brand would never have reached the master — A1 one layer over, now resolved like a run. All eight goldens re-ran at zero tolerance unmoved |
...
- **`config_store_brands.in_pipeline_contract`** is the column that keeps this a migration rather than a behaviour change — see 1.3. *Dropped on 2026-08-21 (migration `022`)*: D12 reconciled the two mappings, so there is nothing left for it to distinguish.
...
- ✅ **`brand_map.csv` is imported but flagged `in_pipeline_contract = false`.** Rendering its 60 rows into `store_to_brand` would change the brand of **28 stores** inside what must be an output-identical refactor. Reconciling the two mappings is D12's own commit with its own stated delta — **done 2026-08-21, and the 28 was wrong**: `derive_brand` matched exactly, the CSV's keys were normalised spellings and the pipeline's are roster spellings, so rendering verbatim would have rebranded **2** storefronts and silently left 40 unbranded. The column is dropped (migration `022`); see the D12 row above
...
fall-through of the storefront→brand mapping (`config/brand_map.csv` at the time; the contract's `store_to_brand` since 2026-08-21). [07-VERIFICATION](07-VERIFICATION.md)
