...
**3b. Cross-window orders** (TikTok and Shopee, between classify and the explode). An order settled in this window may have been *created* earlier, so its SKU lines are in a previous window's order export and the explode's join finds nothing — the revenue then leaves through the "~21% unmatched" door, which is expected to have traffic and so says nothing. `src/backfill.py` looks for those lines in the **nearest same-month predecessor window**, one window per order, never pooled ([D59](06-DECISIONS.md#d59)). `cross_window_order_backfill` decides what happens next: `off` skips the step entirely, `report` (current) measures it and changes no number, `apply` puts the lines in the frame and moves cells. Lazada has no order files, so this cannot happen there.
