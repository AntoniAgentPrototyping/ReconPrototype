...
- **Config stays git-backed YAML.** The UI proposes structured edits and commits them with comments preserved; the database holds proposals and the audit trail. Moving the source of truth into a database would destroy the evidence comments and make month-end depend on the app being up ([D2](06-DECISIONS.md#d2)).
