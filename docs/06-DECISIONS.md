...
*Deliberately not done:* stamping `ROSTER: n expected store(s) absent` into the finance workbook's control block, so the artefact the team invoices from carries its own caveat. That is the stronger control and it **moves workbook cells** — it needs its own commit and a deliberate rebaseline, so it is deferred rather than smuggled into a change that must be output-identical. *(Closed 2026-08-21: the stamp shipped as its own change with exactly the pre-stated four-golden rebaseline — see [D63](#d63).)*
...
### D59 — An order's lines are borrowed from ONE earlier window, never pooled {#d59}
[Defect 2.12](08-KNOWN-DEFECTS.md): an order settled in `w2` may have been created days earlier, so its SKU lines sit in `w1`'s export, the explode's join finds nothing, and the revenue leaves the invoice through the documented "~21% unmatched" door — quietly, because that door is expected to have traffic. July's first external month-end comparison measured **4,527,401,608 VND** going out that way.

**The rule.** For each `(store, order_id)` the income settles but this window's order files do not contain, take that order's SKU lines from the **nearest same-month predecessor window** that has them — exactly one window per order; within that window, every file's lines.

*One window per order.* A re-pulled order file appears in several windows (TikTok re-ships each store's prior-month pull in every weekly folder, deliberately, because the cross-period stitch needs it). Summing every window that holds a copy is the pooling anti-fix, and it is **measured, not feared**: pooling July's TikTok order exports into `w2` takes that window to 183,102,704,362 VND against a reference of 40,060,544,029 — a **4.5× over-count**. Worse, it is invisible: `explode_to_sku_*` sums quantity per `(store, order_id, sku_id, sku_name, unit_price_gross)` bucket, so a duplicate copy inflates quantity *inside* an existing SKU line and adds no row anyone could count.

*Within one window, all files' lines are kept* — the same rule the window's own part-files already follow, because `dedupe_rows: false` and an order legitimately contains two byte-identical SKU lines (a normal unit and a gift variant). Row content cannot tell that apart from a re-pull; the discriminator is **file-level provenance** ([D5](#d5)).

*Predecessors only.* Borrowing from a later window would make output depend on what has arrived since, so re-running `w2` after `w5` was staged could produce a different invoice from the same inputs. Reproducibility beats recall.

*Income is never re-read.* Which settlement rows belong to a window is already decided and verified by `window_settlement_bounds` ([D9](#d9)). Measured on the trap this could have walked into: of the 24,555 rows the `2026-07_w2` bound drops, **24,546 of 24,546 distinct order ids are already in w1** — zero unique.

**Three modes, one code path** — `cross_window_order_backfill: off | report | apply`. `off` is byte-for-byte the behaviour every committed golden was produced under. `report` measures and names it (a tie-out INFO row, an exceptions sheet, log lines) and **changes no number**: the orders frame handed to the explode is the window's own, and none of report mode's outputs is a field of the golden manifest — INFO rows do not become variances, and the manifest's `stage_row_counts` come from an explicit list of fingerprinted functions that this does not touch. `apply` concatenates the borrowed lines before the explode and moves cells.

The default was `report` from 2026-08-19 and is **`apply` from 2026-08-20**. `off` was never the default, deliberately: it would have left the path unexercised until the day it was first trusted with money, which is the worst possible day to discover it. The report-then-apply sequence is what that buys — the computation that moved the money had already been running in production, and its own outputs pre-stated what the flip would do.

**The flip's evidence.** Zero golden cells moved, and that was predicted rather than discovered: `needed` is derived from `read_parts` frames on both sides, so no golden window borrows anything (`w1`/`s1` have no predecessor; `s2`/`s3` have 100% own-window coverage; Lazada has no order files). No golden was re-baselined. On real data `2026-07_w2` applied 942,869,056 VND across 825 orders with **per-order conservation exact** (variance 0.00 on a 38,609,498,443 VND rebuilt total) — the safety property below, holding on production data rather than a fixture.

**The safety property, and why this is not pooling with extra steps.** Borrowed orders enter the tie-out's *matched* population, so TikTok's per-order settlement conservation (1 VND tolerance) and Shopee's revenue crossing run **over the borrowed lines**. A predecessor re-export whose quantities had drifted therefore breaches a check instead of silently mis-invoicing. The lines are pulled through the same controls the window's own lines pass.

**Borrowed lines are read through the pipeline's own rules, both halves of them** (added 2026-08-20). `ingest.read_files` gives the reading rules; `ingest.normalize_parts` gives the post-read ones — numeric coercion, date parsing, `store_aliases`. Sharing only the first was a real defect: `needed` is built from frames that went through `read_parts`, so it holds *canonical* store names, while a filename holds whatever the platform exported. `settings.yaml` maps `"Pediasure" -> "Abbott Pediasure"` because the order files drop the "Abbott", so the file prefilter skipped `w1`'s Abbott exports and 941,081,056 VND of July's measured recovery reported as **zero** in report mode. The filename prefilter therefore goes through `ingest.canonical_store` too — the same function the roster preview uses, so a store cannot be called one thing by the roster and another by the borrow.

**One failure policy, and the mode selects which half.** An unreadable or ambiguous predecessor **warns and skips that window under `report`, and refuses under `apply`** — the same rule in `src/backfill.py` and `service/materialize.py`. The asymmetry is the point: report mode's contract is that it changes nothing, and a settlement run that *dies* over a sibling window's corrupt file has changed a great deal, while under `apply` those same bytes are about to become invoice lines. Under `apply` a failure that is not already a `ReconHardStop` is re-raised as one, because pandas' "Worksheet named 'OrderSKUList' not found" names no remedy to the person reading it. Before this, the three cases disagreed with each other: a digest mismatch hard-stopped a report-mode run while an unnameable file and a missing object beside it only warned, and two predecessor uploads sharing a filename were resolved silently last-one-wins where the window's own files refuse exactly that collision.

**What no mode can recover.** July's `purite` and `mondelez kinh do` cells are byte-identical mis-pulled order files (sha `9bd3750061f4`, `f7e3dda0d99a`) — the lines were never exported by the platform, so they exist in no window. The remedy is a re-pull, which the upload door and the coverage report now surface at upload time ([D57](#d57), [D58](#d58)) instead of at month-end.

### D60 — The database is the config audit record; git is a developer-checkout convenience {#d60}
Since M8/1.2 every applied config change lands as a `config_versions` row: content-addressed (sha256), append-only, naming who proposed, who approved and when, with the full rendered text. The api ALSO writes `settings.yaml` to disk and commits it where a git checkout exists — but the deployed containers have no `.git` (`.dockerignore` excludes it, deliberately), so that half silently no-oped and the API reported `committed: false` with no explanation, while `docs/10-ROADMAP.md` still stated the design premise as "config stays git-backed YAML" ([C11](14-PRODUCTION-READINESS.md)).

**Decided (2026-08-20): the database row is the audit record.** The git commit is the reviewable convenience of the developer checkout — nice where it exists, not a control anywhere. The API response now says so (`audit_record` names the version row; `git_note` explains a null commit), and the config page renders the explanation instead of a bare "not a git checkout".

**Rejected: pushing config commits to a remote from the containers.** It would put a writable git credential into every api container — a new secret surface (C13's shape) — to duplicate a record the database already keeps with strictly more metadata; and a push that fails at month-end would either block a rate change or silently diverge, recreating the exact ambiguity this decision removes. The committed `config/settings.yaml` in git remains what it already was: the SEED a fresh deployment imports, exported back by `service.admin config export` when a human wants the diff in a review.

### D61 — An exception disposition annotates; it never hides {#d61}
[Register D1](14-PRODUCTION-READINESS.md): the stable fingerprint existed since M5 and history was queryable, but nothing could attach a decision to a fingerprint, so every run re-presented the same list as if nobody had ever looked. Migration `020` adds `exception_dispositions` (current state per fingerprint) and `exception_disposition_events` (append-only mark/clear history) — the same two-table shape as the config pins (migration `014`, [D2](#d2)'s audit-trail lineage), because it is the same problem: a judgment call that must survive across runs, attributed, with a reason, never silently lost.

**Decided (2026-08-21): a disposition is a badge, never a filter the reader did not ask for.** The queue's default answer always carries every row, dispositioned or not; `open_only` is an explicit filter a screen offers. The reason is structural: the fingerprint hashes IDENTITY columns, not amounts, so an "expected" variance that has quietly grown produces the same fingerprint — auto-suppressing recurrences would hide exactly the change a reviewer must see. Same transparency rule as `truncated` on the sheet counts.

**Who may disposition: `recon.user`, not admin.** Routine month-end triage, done by the person who knows why a row is expected; the control is the mandatory reason (8-character floor, the roster declaration's) and the append-only event trail, not the rank. Clearing requires its own reason and records what was released — re-opening a decision is as consequential as making one (the unpin pattern).

**No TTL, stated rather than left open.** With annotate-never-hide the row resurfaces every run anyway; an expiry would only manufacture re-approval work. `decided_at` is on the badge, so a stale decision is visible for a person to reconsider.

**Rejected: keying the disposition to a run.** The decision is about the recurring thing ("this store's unmatched orders are known and excluded"), not one week's copy of it — keying per run would recreate the re-triage-every-run problem this exists to close. The fingerprint is deliberately not an FK anywhere, so no retention policy can cascade the history away.

### D62 — The roster declaration names its stores {#d62}
[Register D3](14-PRODUCTION-READINESS.md), first clause: `apply_partial_roster` took only a platform, so a partial declaration optionalized EVERY expected store and a genuinely forgotten store was waved through with the legitimately absent ones. Migration `021` adds `windows.declared_absent_stores text[]`; `apply_partial_roster(settings, platform, optional_stores)` relaxes only the named stores, and `check_stores`' unchanged set arithmetic hard-stops any other absent store again — the fix is a narrower input to the existing control, not a new control.

**A column, not a child table.** The declaration is one act — one person, one reason, one timestamp, upserted whole — and per-store rows would imply per-store reasons and lifecycles nobody asked for. There is also nothing to foreign-key against: the roster lives in the config contract.

**NULL is the legacy blanket, kept expressible.** Every declaration made before the column existed means "all stores optional", and a re-run must make the same claim the first run did — so the blanket keeps working, the worker's run log names it as a blanket with a nudge to narrow it, and the UI offers the store list as the upgrade path. A declaration also legitimately predates knowing which stores will be missing. The developer CLI (`--partial-roster` on `tools/devrun.py` / `tools/make_golden.py`) stays a bool and therefore blanket-mode: that is the mode every partial-roster golden is regenerated in, and it must stay byte-identical (pinned by `tests/test_partial_roster.py`).

**Membership validates at run time, against the run's own roster — never at the API door.** The window may be pinned to an older config whose roster differs from today's, so the door cannot know which list applies. A declared name the run's roster does not know is a `ReconHardStop` naming the store, not a silent skip: skipping would resurrect the misleading "missing store" stop one step later.

**Re-evaluation is legibility, not money — deliberately.** A declared-absent store that later HAS files is merely present-and-optional; its figures are in every total already. So the stale declaration is a run-log warning (worker-side, never a `RunResult` finding — findings come from `src/` over the frames it reads) plus an amber notice on the window page, whose upload-plan payload now carries `declared_absent_present` — the page re-renders after every upload action, so it IS the upload-time hook. *Deliberately not built:* automatic withdrawal of a stale declaration (a control must not edit itself) and a persisted staleness table (the live sets recompute in milliseconds; storing them creates a second answer that can drift).

### D63 — The workbook says it is partial {#d63}
Closes [D46](#d46)'s deferral, as its own change with the delta stated in advance. A partial-roster run now stamps two cells into each platform's primary control tab (`PV sum` A1/B1 on TikTok and Shopee, `Summary` A1/B1 on Lazada): the label `Roster` and a Vietnamese sentence naming the absent stores — Vietnamese because the workbook's own verdicts are (`VERDICT_OK`/`VERDICT_BAD`), and NAMES rather than a count because store names already appear throughout the file and a count sends the reviewer back to the system the workbook exists to stand apart from.

**Relaxed-set-based, never `expected − found`.** The stamp reads `RunContext.roster_relaxed` (the declared list, or the whole roster in blanket/CLI mode; empty when not partial), threaded through `meta`. A config-level `stores_optional` store absent from a non-partial window must NOT stamp — the four non-partial lazada goldens regenerated with zero refusals as the proof. Deliberately **not** a `checks` entry: a checks entry would move `fingerprint_digest` alongside the workbook digest, and the stamp is a caveat about coverage, not a verdict about money.

**The golden delta was pre-stated and matched:** only the four `partial_roster: true` windows (`2026-05_s1/s2/s3` shopee, `2026-05_w1` tiktok) moved, by exactly the two stamp cells each, `fingerprint_digest` and `variances_digest` unchanged, rebaselined with `--rebaseline --reason` naming this decision.

**Row 1, and why:** a caveat should be the first thing read — and it is also the only safe region, because `_Tab.emit` silently drops buffered control cells at rows ≥ `data_start_row` on tabs that emit data, which is a pre-existing latent defect recorded the same day ([defect 2.13](08-KNOWN-DEFECTS.md)): the TikTok `PV sum` side-block totals and verdicts (rows 5–8) and Shopee's (rows 5–11, including the `J11` working verdict) have never reached the file. That defect is recorded, not fixed here — fixing `emit` moves cells on all eight goldens and needs its own pre-stated delta.

### D64 — A contract-critical setting has no code default {#d64}
[Register A9](14-PRODUCTION-READINESS.md). `settings.get(key, default)` embeds a claim about the configured value in the code that reads it, and the claim goes stale on its own. Three keys — `dedupe_rows`, `drop_unmapped_columns`, `cross_window_order_backfill` — now have no default at all: `src.config.require` hard-stops naming the key, what the value decides, and where to set it.

**The second inversion is the argument, not the first.** The register named `dedupe_rows` (default `True`, contract `false` — legitimate duplicate gift-SKU lines dropped, revenue understated). The one that settles the principle was found while planning the fix: `cross_window_order_backfill` defaulted to `"off"` against a contract that says `apply`, silently reverting [defect 2.12](08-KNOWN-DEFECTS.md)'s fix — 942,869,056 VND on `2026-07_w2`, 1,390,095,674 on `_s4`, 173,429 on `_s2`. That default was **correct and safe on the morning it was written**, when the mode still was `off`, and wrong by that evening when the mode was flipped. `mode_of` guarded the *typo* case with a comment reading "unset and misspelled are different mistakes" and left the likelier mistake unguarded, and a test asserted the stale behaviour citing "today's behaviour, the one every committed golden was produced under" — a claim about the world, not the code.

**Read-time and render-time refusals share one set of keys.** `service/config_render.DANGEROUS_DEFAULTS` imports `src.config.REQUIRED_SETTINGS` rather than restating it: `service/` may import `src/`, and two spellings of "these keys have no safe default" is how one goes stale — which is exactly what happened to the old local copy, whose text described the backfill default as "the SAFE direction". Render-time refusal is kept, not replaced: a run that hard-stops has already been queued, materialised and started.

**Quoted booleans are refused too.** `config_scalars.value` is `jsonb` and `settings.yaml` is text, so `false` can arrive as the string `"false"` — truthy in Python, a silent inversion of a PII or revenue flag. The editor guards this at the row (M8/1.6); `require` is the same guard on the path that has no rows, which is the CLI and any hand-edited file.

**What is NOT required, stated so it is not re-litigated.** `numeric_coercion`, `number_style` and `file_formats` agree with their defaults and absence is the strict direction. `header_rows`/`skip_rows_after_header` are shape defaults, not policy. `date_coercion` is not in the contract at all and its `warn` default is the deliberate posture ([D53](#d53)) — modelling it as a row changes what an operator can do and is not this decision's business. `dayfirst.<platform>` IS an inversion (`tiktok: true` against a `False` default) and a dead one: `date_formats` decides for both platforms that read it ([D54](#d54)), and the walk that enforces this decision cannot compare a per-platform default against a scalar, so it says so rather than pretending to cover it.

**The enforcement is a test, because the fix alone closes one instance of a class that has now appeared twice.** `tests/test_config_defaults.py` pairs every surviving `settings.get(<key>, <literal>)` in `src/` with `config/settings.yaml` and fails on disagreement; a key nothing configures must be allow-listed with its reason. It carries a negative control that parses both historical inversions as source text — a walk that silently matched nothing would make the whole file pass, which is the failure mode `xfail(strict=True)` exists to catch elsewhere.

**Expected delta: none, and measured.** All eight golden windows re-ran under DB-rendered config at zero tolerance with no cell moved.

### D65 — One storefront→brand mapping, in the contract {#d65}
[Register D12](14-PRODUCTION-READINESS.md). There were two: `store_to_brand` in `config/settings.yaml` — `{}`, so `ingest.derive_brand` fell back to the store name for every store and warned on every run — and `config/brand_map.csv`, 60 rows read only by the month-end master. `config_store_brands.in_pipeline_contract` (migration `007`) marked which was in force, and existed so that the config migration could be output-identical. Reconciling them was always going to be its own change; this is it. One table, one rendered key, two projections, and the CSV is deleted.

**Flipping the flag would have been the wrong fix, and measuring said so.** `derive_brand` used an exact `df["store"].map(mapping)`. brand_map.csv's keys were `norm_store` spellings (`abbott grow`, `ufood store`); the pipeline's stores are roster spellings (`Abbott grow`, `ufood_store`). Exact hits: TikTok **0 of 25**, Shopee **2 of 17**, Lazada 0 of 18. So rendering the rows verbatim would have rebranded two storefronts and silently left forty unbranded — with the contract looking fully populated, which is worse than either the empty map or a complete one. Migration `007`'s own comment said "28 stores would change"; that figure was measured a different way and was never what the pipeline would have done.

**So the fix is the matching rule, not the flag.** Both readers resolve through `ingest.norm_store` on both sides — `store_brands(settings, platform)` for a run, `brand_map(settings)` for the master, two projections of one function. Under it all 25 TikTok and all 17 Shopee rows match the roster. `norm_store` moved from `pipeline` to `ingest` to make that possible (`pipeline` imports `ingest`, not the reverse) and is re-exported, because a second copy of store identity is what A13 already had to delete once.

**Per platform, and with the review metadata.** The rendered shape is `store_to_brand.<platform>.<store> = {brand, confidence, note?}`. Per platform because the table's key is `(platform, store)` and a flat map collapses a storefront selling under different brands on two platforms — lossless today (measured: no conflicting flat keys) and not a property to depend on. `confidence` and `note` are contract rather than editor decoration because `src/master_summary.py` paints all three on the master's "Brand mapping" tab, which is the surface the team reviews the mapping on; leaving them in the database would have made the CLI's master unable to ask for a review. **The shape change was free exactly now**: the key was `{}`, so every pinned pre-change config renders an empty map and no window's behaviour depends on the old shape. Once it has rows, changing it means repinning.

**A bare string is refused.** `store_to_brand.tiktok.Kao: "KAO"` is the natural thing to hand-write and it is a hard stop, because reading it would mean inventing a `confidence` — and the entire point of that field is that nobody has to guess whether a mapping was reviewed.

**A brand row must name a roster storefront**, the same rule `config_store_aliases` already carries and for the same reason: a mapping keyed on a spelling no run produces is configured and inert, and inert config reads as a decision. Matched through `norm_store` there too, so a person may type `ufood store` where the roster says `ufood_store`. **Lazada is exempt and reported, not refused** — `expected_stores.lazada` is empty (register A6, a business question), and `_roster` returning nothing means "nobody has said", so its 18 rows are accepted.

**What deliberately did NOT get filled in.** Three Shopee storefronts (`Tolpa`, `pepsicofoods`, `xa_kho_gia_tot`, the July onboardings) have no brand row, because nobody has told us their client brand. They fall back to their own name, the run reports it, and the master flags them UNMAPPED for review. Inventing a brand in a mapping the month-end master invoices from is not engineering's to do — the same posture as the unresolved alias sentinel and `expected_stores.lazada`.

**One vocabulary for `confidence`.** The master flagged any value that was not `high` — brand_map.csv's word — while the table's CHECK constraint allows `confirmed`/`needs_confirmation`. Left alone, every row would have printed red the moment the CSV became the contract. The database's words win; the master now flags anything that is not `confirmed`.

**The delta, stated before the gate and measured after.** `derive_brand` starts assigning real brands to 42 storefronts instead of falling back to their names, and the per-run "no `store_to_brand` mapping" warning goes quiet for them. **No workbook cell may move**, because `brand` is functionally determined by `store` — so `groupby(["store", "brand", ...])` in `src/classify.py` has identical cardinality and ordering — and no `finance_template` cell reads it (its "brand tabs" are `bucket`-driven, which is `invoice_buckets`, a different rule). Had a cell moved, that would have been the finding: `brand` reaching output means D12 is a bigger change than this decision claims.

### D66 — The store is confirmed before the bytes are sent, by the pipeline's own rule {#d66}
[Register D7](14-PRODUCTION-READINESS.md). `POST /uploads` has taken an optional `store` since M6 — to *confirm or correct* what the filename pattern found, recording both values — and `web/app/actions.ts` has posted `store:<filename>` per file for just as long. **Nothing ever rendered an input**, so the affordance the code documented was unreachable and an operator whose file parsed to the wrong storefront had exactly one recourse: rename it on disk and upload again.

**`POST /uploads/store-preview` — filenames only, and that is the whole design.** The cost this exists to avoid is a 184 MB export uploaded to the wrong storefront: a reject, a re-upload and an explanation. Sending the bytes to find out would make the preview cost the thing it saves. It answers per filename with the derived store, its canonical form after aliases, the uniform name the file will carry, whether that storefront is on this window's roster, and the refusal sentence if the name cannot be read at all.

**The derivation is the API's, never the browser's.** Store identity comes from the filename ([D6](#d6)), so a regex in `web/` would be a second definition of the rule that decides whose revenue a file becomes — the most damaging drift this system could acquire, and the one thing `src/ingest.store_from_filename` exists to be the only copy of. The route calls `naming.store_of`, which is what the upload and the run call. `tests/test_upload_store_control.py` asserts that no `.tsx` file reads `store_from_filename` outside a comment.

**A POST that changes nothing, at role `user`.** POST because the input is a list of filenames and a filename is not something to length-cap by URL — and because a GET would spend every storefront's filenames into the api access log for no reason (business identifiers, not customer PII, but there is nothing to gain). `user` rather than `viewer` even though it is read-only: it is a step in uploading, and a viewer cannot upload. Keeping it at the same role as `POST /uploads` means "who may upload" has one answer.

**It previews against the config the WINDOW will run under** (`_domain_for_window`), not today's disk copy — the same rule `/uploads/plan` already follows, and for the reason M6 found by running the demo through the containers: a pinned window's roster is a different list, and a preview that disagrees with the control is worse than no preview.

**A picklist where a roster exists, free text where none does.** A store the roster does not name is refused at the door anyway and the fix is a config proposal, so a free-text box would produce the same refusal more slowly. Lazada has no roster at all (register A6) and its door accepts-and-reports, so there the correction is a text field — the alternative is a picklist with nothing in it. *Measured, so the picklist cannot offer a self-refusing option:* all 45 roster storefronts survive `validate_roundtrip` on every kind their platform has, so no roster name would be accepted by the form and then refused by the door.

**Reporting, not refusing, on a per-file problem.** An unreadable name or an off-roster store comes back as a state on that row while every other row still answers. A 422 for the batch would hide the answers for the files that were fine, which is the batch the operator asked about.

**A failed preview does not block the upload.** The door performs the same checks and refuses there with the same sentences; the preview is a way to find out early, not a new control. So `previewStores` returns its error to the form and the form stays usable — a preview outage must not stop month-end.

**Still not verified by a browser.** Register E2 stands: this was typechecked and reasoned about, and no person or test has driven the screen. What is tested is the route (five cases in `tests/service/test_uploads.py`) and the field name in all three places, which is the side that actually went missing.

### D67 — Format drift is reported where it is seen, and the fix is one step away {#d67}
[Register D5](14-PRODUCTION-READINESS.md). [12-CHANGE-HISTORY](12-CHANGE-HISTORY.md) logs 18 drift events in three months and **every one needed a developer**: the platform renamed a column, the run hard-stopped ~200 seconds in with *"income data is missing required columns after header mapping: ['net_revenue']. Update column_maps.income in settings.yaml"*, and somebody who knew what a column map was edited YAML. The rules editor could always express the fix. Nothing helped a person *find* it.

**Two halves, and neither is a new control.** The evidence: which headers a file carries that the contract does not name, per file, PII subtracted. The step: map one of them to a canonical field as a config proposal, from the window screen. `POST /jobs` still refuses, `read_parts` still hard-stops, propose→approve→apply and `invalidates_goldens` still stand between the form and a settlement run. What changed is that the operator learns at upload time instead of mid-run, in a sentence instead of a traceback.

**The required-field arithmetic is per WINDOW-AND-KIND, never per file, and that is the load-bearing decision.** `ingest.read_parts` concatenates a kind's part files and checks its required columns against the *result*, so a "part 2" export carrying fewer columns is legitimate — July produced nine of them. A per-file check at the upload door would have refused healthy windows to catch a fault the union does not have. So `uploads.missing_fields` takes the union across the kind's files and lives on `/uploads/plan`, where it appends to `problems` and therefore makes `ready` false: a window a run will refuse must not look ready. `tests/service/test_uploads.py::test_a_part_file_with_fewer_columns_is_not_a_drift_report` pins it from both sides.

**PII is subtracted from the evidence, or the report is worthless.** Every healthy export carries `Recipient`, `Phone #` and `Detail Address`, and the contract deliberately does not name them. A drift list including those fires on every file forever, and an operator learns to ignore it — the same failure `src/pipeline.py:576-580` predicted for the UNVERIFIED list, and the reason [A3](14-PRODUCTION-READINESS.md) had to be fixed at all.

**"Clean" and "unmeasured" are different answers.** `checked: false` means nothing was compared: Lazada's kinds have no entry in `REQUIRED_COLUMNS` (it is a fee-event ledger read by `lazada.read_ledger`), and a file uploaded before migration `023` recorded no headers. Reporting either as "no drift found" would be the shape of every control this register has had to fix.

**`store` is excluded from the required set.** It is in `REQUIRED_COLUMNS` and comes from the *filename* ([D6](#d6)) — `read_files` adds it before `read_parts` checks — so testing a file's own headers for it would fail every export ever written. `PIPELINE_SUPPLIED_FIELDS` names that rather than leaving it as a subtraction somebody has to rediscover.

**Raw headers are stored, not the fields they mapped to** (migration `023`). The mapping belongs to the contract, which is versioned and pinned per window; a stored mapping would freeze one reading of the file and go stale the moment a column map is edited. The file said what it said — that is the durable fact, and every consumer maps it through the config the window actually runs under. `pii_columns_dropped` already held every dropped header despite its name, so `kept_columns` completes the original header row.

**No suggested mapping, and this is where that was decided.** The screen shows the unknown headers and the fields nothing supplies; the human pairs them. Ranking candidates by name similarity is what [D7](#d7) exists to refuse — `Pediasure` → `Abbott Pediasure` was accepted on order-ID-overlap proof, and a storefront that *looked* like an alias was proven genuinely new at zero overlap. Suggesting the pairing is the first thing in this system that would call a model, it needs to read cell **values** (which are PII-bearing), and it is deferred to its own pass with the constraints written down in [10-ROADMAP](10-ROADMAP.md#where-ai-does-and-does-not-belong).

**Canonical field names render untranslated on a translated page**, deliberately: `unit_price_gross` is the same identifier the rules editor's own dropdown shows, and inventing a Vietnamese label for it would create a second name for one thing — the boundary [Phase 5](14-PRODUCTION-READINESS.md) already drew when it left the rules editor in English.
